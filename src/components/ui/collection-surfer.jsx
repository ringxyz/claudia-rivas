...
function Card({index,progress,en,onOpen}){
 const z=useTransform(progress,[0,1],[-index*240,(7-index)*240]);
 const x=useTransform(progress,[0,1],[index*190,(index-7)*190]);
 const y=useTransform(progress,[0,1],[-index*58,(7-index)*58]);
 const number=String(index+1).padStart(2,'0');
 return <motion.figure className="surfer-card" style={{x,y,z,rotateY:-32}}><button className="surfer-open" onClick={()=>onOpen(index)} aria-label={`${en?'Open photograph':'Abrir fotografía'} ${index+1}`}><img src={asset(`media/model-${number}-480.webp`)} srcSet={`${asset(`media/model-${number}-480.webp`)} 480w, ${asset(`media/model-${number}-960.webp`)} 960w`} sizes="(max-width:700px) 78vw, 320px" width="480" height="640" loading="lazy" alt={label(index,en)}/><span aria-hidden="true">↗</span></button><figcaption>{number} / 08</figcaption></motion.figure>;
}
...
function Lightbox({index,en,onClose,onChange}){
 ...
 const number=String(index+1).padStart(2,'0');
 return createPortal(<div className="lightbox" role="dialog" aria-modal="true" aria-label={en?'Fashion gallery':'Galería de moda'} onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><button ref={closeRef} className="lightbox-close" onClick={onClose} aria-label={en?'Close gallery':'Cerrar galería'}>×</button><button className="lightbox-arrow lightbox-prev" onClick={()=>onChange(-1)} aria-label={en?'Previous photograph':'Fotografía anterior'}>←</button><figure><img src={asset(`media/model-${number}.webp`)} alt={label(index,en)}/><figcaption>{number} / 08</figcaption></figure><button className="lightbox-arrow lightbox-next" onClick={()=>onChange(1)} aria-label={en?'Next photograph':'Fotografía siguiente'}>→</button></div>,document.body);
}
